...
local function digestblock(w, C)
    for j = 17, 64 do
        local v = w[j-15]
        local s0 = bxor(rrotate(w[j-15], 7), rrotate(w[j-15], 18), sha_brshift(w[j-15], 3))
        local s1 = bxor(rrotate(w[j-2], 17), rrotate(w[j-2], 19),sha_brshift(w[j-2], 10))
...
local function toBytes(t, n)
    local b = {}
    for i = 1, n do
        b[(i-1)*4+1] = band(sha_brshift(t[i], 24), 0xFF)
        b[(i-1)*4+2] = band(sha_brshift(t[i], 16), 0xFF)
        b[(i-1)*4+3] = band(sha_brshift(t[i], 8), 0xFF)
...
local function pbkdf2(pass, salt, iter, dklen)
    ...
    while dklen > 0 do
        ...
        isalt[#isalt+1] = band(sha_brshift(block, 24), 0xFF)
        isalt[#isalt+1] = band(sha_brshift(block, 16), 0xFF)
        isalt[#isalt+1] = band(sha_brshift(block, 8), 0xFF)
...
local sha256 = {digest = digest, hmac = hmac, pbkdf2 = pbkdf2}
...
local function textToBigint(text)
...
local function bigintToText(num)
    local bytes = {}
    local zero = bigint(0)
    local two56 = bigint(256)
    while num > zero do
        local byteBig = num % two56
        local byte = 0
        local temp = clone(byteBig)
        while temp > bigint(0) do
            temp = temp - bigint(1)
            byte = byte + 1
        end
        table.insert(bytes, 1, string.char(byte))
        num = divint(num - byteBig, 256)
    end
...
local function rsaEncrypt(message, publicKey)
	...
	return tostring(modexp(m, e, n))
end

local function rsaDecrypt(ciphertext, privateKey)
    local n = bigint(privateKey.shared)
    local d = bigint(privateKey.private)
    local c = bigint(ciphertext)
	local m = modexp(c, d, n)
...
local function str2ints(s)
	return {
		b2uint32({string.byte(s, 1, 4)}),
		b2uint32({string.byte(s, 5, 8)}),
		b2uint32({string.byte(s, 9, 12)}),
		b2uint32({string.byte(s, 13, 16)}),
	}
end

local function ints2str(b)
...
local function encryptBlock(xk, src)
	local s0, s1, s2, s3 = src[1], src[2], src[3], src[4]
	...
	local t0, t1, t2, t3
	for _ = 1, nr do
		t0 = bxor(xk[k+1], bxor(te0[uint8(brshift(s0, 24))], bxor(te1[uint8(brshift(s1, 16))], bxor(te2[uint8(brshift(s2, 8))], te3[uint8(s3)]))))
		t1 = bxor(xk[k+2], bxor(te0[uint8(brshift(s1, 24))], bxor(te1[uint8(brshift(s2, 16))], bxor(te2[uint8(brshift(s3, 8))], te3[uint8(s0)]))))
		t2 = bxor(xk[k+3], bxor(te0[uint8(brshift(s2, 24))], bxor(te1[uint8(brshift(s3, 16))], bxor(te2[uint8(brshift(s0, 8))], te3[uint8(s1)]))))
		t3 = bxor(xk[k+4], bxor(te0[uint8(brshift(s3, 24))], bxor(te1[uint8(brshift(s0, 16))], bxor(te2[uint8(brshift(s1, 8))], te3[uint8(s2)]))))
		k = k + 4
		s0, s1, s2, s3 = t0, t1, t2, t3
	end

	s0 = bor(blshift(sbox0[uint8(brshift(t0, 24))], 24), bor(blshift(sbox0[uint8(brshift(t1, 16))], 16), bor(blshift(sbox0[uint8(brshift(t2, 8))], 8), sbox0[uint8(t3)])))
	s1 = bor(blshift(sbox0[uint8(brshift(t1, 24))], 24), bor(blshift(sbox0[uint8(brshift(t2, 16))], 16), bor(blshift(sbox0[uint8(brshift(t3, 8))], 8), sbox0[uint8(t0)])))
	s2 = bor(blshift(sbox0[uint8(brshift(t2, 24))], 24), bor(blshift(sbox0[uint8(brshift(t3, 16))], 16), bor(blshift(sbox0[uint8(brshift(t0, 8))], 8), sbox0[uint8(t1)])))
	s3 = bor(blshift(sbox0[uint8(brshift(t3, 24))], 24), bor(blshift(sbox0[uint8(brshift(t0, 16))], 16), bor(blshift(sbox0[uint8(brshift(t1, 8))], 8), sbox0[uint8(t2)])))
...
local function decryptBlock(xk, src)
	local s0, s1, s2, s3 = src[1], src[2], src[3], src[4]
	...
	local t0, t1, t2, t3
	for _ = 1, nr do
		t0 = bxor(xk[k+1], bxor(td0[uint8(brshift(s0, 24))], bxor(td1[uint8(brshift(s3, 16))], bxor(td2[uint8(brshift(s2, 8))], td3[uint8(s1)]))))
		t1 = bxor(xk[k+2], bxor(td0[uint8(brshift(s1, 24))], bxor(td1[uint8(brshift(s0, 16))], bxor(td2[uint8(brshift(s3, 8))], td3[uint8(s2)]))))
		t2 = bxor(xk[k+3], bxor(td0[uint8(brshift(s2, 24))], bxor(td1[uint8(brshift(s1, 16))], bxor(td2[uint8(brshift(s0, 8))], td3[uint8(s3)]))))
		t3 = bxor(xk[k+4], bxor(td0[uint8(brshift(s3, 24))], bxor(td1[uint8(brshift(s2, 16))], bxor(td2[uint8(brshift(s1, 8))], td3[uint8(s0)]))))
		k = k + 4
		s0, s1, s2, s3 = t0, t1, t2, t3
	end

	s0 = bor(blshift(sbox1[uint8(brshift(t0, 24))], 24), bor(blshift(sbox1[uint8(brshift(t3, 16))], 16), bor(blshift(sbox1[uint8(brshift(t2, 8))], 8), sbox1[uint8(t1)])))
	s1 = bor(blshift(sbox1[uint8(brshift(t1, 24))], 24), bor(blshift(sbox1[uint8(brshift(t0, 16))], 16), bor(blshift(sbox1[uint8(brshift(t3, 8))], 8), sbox1[uint8(t2)])))
	s2 = bor(blshift(sbox1[uint8(brshift(t2, 24))], 24), bor(blshift(sbox1[uint8(brshift(t1, 16))], 16), bor(blshift(sbox1[uint8(brshift(t0, 8))], 8), sbox1[uint8(t3)])))
	s3 = bor(blshift(sbox1[uint8(brshift(t3, 24))], 24), bor(blshift(sbox1[uint8(brshift(t2, 16))], 16), bor(blshift(sbox1[uint8(brshift(t1, 8))], 8), sbox1[uint8(t0)])))
...
local function subw(w)
	return bxor(blshift(sbox0[uint8(brshift(w, 24))], 24),
		bxor(blshift(sbox0[uint8(brshift(w, 16))], 16),
		bxor(blshift(sbox0[uint8(brshift(w, 8))], 8),
		sbox0[uint8(w)])))
end

local function rotw(w)
	return bor(blshift(w, 8), brshift(w, 24))
end
...
local function expandKey(key)
	...
	local nk = #key / 4
	for i = 1, nk do
		enc[i] = b2uint32({string.byte(key, (i - 1) * 4 + 1, i * 4)})
	end
	for i = nk, kl - 1 do
		local t = enc[i]
		if i % nk == 0 then
			local t0 = t
			t = bxor(subw(rotw(t)), blshift(powx[i / nk], 24))
		elseif nk > 6 and i % nk == 4 then
			t = subw(t)
		end
		enc[i + 1] = bxor(enc[i - nk + 1], t)
	end
	...
	for i = 0, kl - 1, 4 do
		...
		for j = 1, 4 do
			...
			if i > 0 and i + 4 < kl then
				x = bxor(td0[sbox0[uint8(brshift(x, 24))]], bxor(td1[sbox0[uint8(brshift(x, 16))]], bxor(td2[sbox0[uint8(brshift(x, 8))]], td3[sbox0[uint8(x)]])))
			end
			dec[i + j] = x
		end
	end
...
Cipher = { _enc = nil, _dec = nil, enc_vec = nil, dec_vec = nil }

function Cipher:new(o, key, vec)
	o = o or {}
	setmetatable(o, {__index = self})
	assert(string.len(key) == 16 or string.len(key) == 24 or string.len(key) == 32)
...
function Cipher:encryptB(src)
	assert(#src == 16)
	return ints2str(encryptBlock(self._enc, str2ints(src)))
end

function Cipher:decryptB(src)
	assert(#src == 16)
	return ints2str(encryptBlock(self._dec, str2ints(src)))
end

-- Output feedback mode

function Cipher:encrypt(src)
	local s = fitTo16(src)
	local res = ''
	for i = 1, #s, 16 do
		self.enc_vec = encryptBlock(self._enc, self.enc_vec)
		res = res..ints2str(xor_ints(self.enc_vec, str2ints(string.sub(s, i, i + 15))))
	end
	return string.sub(res, 0, #src)
end

function Cipher:decrypt(src)
	local s = fitTo16(src)
	local res = ''
	for i = 1, #s, 16 do
		self.dec_vec = encryptBlock(self._enc, self.dec_vec)
		res = res..ints2str(xor_ints(self.dec_vec, str2ints(string.sub(s, i, i + 15))))
	end
	return string.sub(res, 0, #src)
end
-- AES ENCRYPTION LIBRARY -------------------------------------------------------------------------------
...
local function registerCallback(id, func, limit, timeout)
    api.callbacks[id] = {
        func = func,
        limit = limit or 1,
        timeout = timeout and (os.epoch("utc") + timeout * 1000) or nil,
        triggers = 0
    }
end
...
function api.broadcast(timeout, callback)
    local message, id = createMessage("network", {action = "discovery"}, nil, false)
	registerCallback(id, function(response)
		...
		api.nations[response.payload.name] = {
			shared = response.payload.key.shared,
			public = response.payload.key.public,
			trusted = trusted
		}
		if callback then callback(response) end
	end, math.huge, timeout or 10)
...
function api.auth(nation, name, password, callback)
    ...
    registerCallback(id, function(response)
        if response.payload.status == "success" then
            ...
            api.channel = response.payload.channel
...
local function handleChannelRotation(newChannel)
	api.modem.close(api.channel)
	api.modem.open(newChannel)
	api.channel = newChannel
end
...
--[[
The event loop does the following things in order:
1. Resolve internal stuff like heartbeats, channel rotation, decryption, etc
2. If the message ID has an associated callback, execute that.
3. If not, forward the message to the "onEvent" parameter function defined by the user
]]--
function api.listen(onEvent)
    while api.active do
        local event, side, channel, replyChannel, raw, distance = os.pullEvent("modem_message")
			...
			if msg then
				...
				for id, cb in pairs(api.callbacks) do
					if cb.timeout and time >= cb.timeout then
						api.callbacks[id] = nil
					end
				end

				local content = msg.message
				if not msg.isPlaintext then
					local key = api.keyStr ~= "" and api.keyStr or api._pendingKeyStr
					local iv = msg.message.iv
					content = textutils.unserialize(Cipher:new(nil, key, iv):decrypt(msg.message.cipher))
				end

				local internal = false
				if content.header == "network" then
				...
				local cb = api.callbacks[msg.id]
				if cb then
					cb.triggers = cb.triggers + 1
					cb.func(content)
					if cb.triggers >= cb.limit then
						api.callbacks[msg.id] = nil
					end
				else
					if not internal then onEvent(content) end
				end
			end
        end
		::skip::
